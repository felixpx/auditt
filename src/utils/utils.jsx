...
export const covalentGetTokenIds = async (contractAddress) => {
  ...
  await axios
    .get(
      `https://api.covalenthq.com/v1/80001/tokens/${contractAddress}/nft_token_ids/`,
      {
        params: {
          "quote-currency": "USD",
          format: "JSON",
...
export const covalentGetTokenMetadata = async (contractAddress, tokenId) => {
  ...
  await axios
    .get(
      `https://api.covalenthq.com/v1/80001/tokens/${contractAddress}/nft_metadata/${tokenId}/`,
      {
        params: {
          "quote-currency": "USD",
          format: "JSON",
...
export const covalentGetMetadataForContract = async (contractAddress) => {
  ...
  tokens.forEach(async (token) => {
    ...
    if (token.token_id != undefined) {
      const data = await covalentGetTokenMetadata(
        contractAddress,
        token.token_id
      );
      if (data != undefined) metadata.push(data);
    }
  });
  return metadata;
};
