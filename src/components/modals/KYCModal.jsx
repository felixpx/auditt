...
export default function KYCModal(props) {
  ...
  const approveKYC = async () => {
    ...
    // ADD LOGIC FOR APPROVING KYC
    setOpen(false);
  };

  useEffect(() => {
    console.log(props.KYCList);
  }, []);
                ...
                <div>
                  ...
                  <div className="mt-3 text-center sm:mt-5">
                    ...
                    <div className="flex flex-col items-center justify-center space-y-2 py-6">
                      <div>{props.KYCList[0].name}</div>
                      <div>{props.KYCList[0].email}</div>
                      <div>{props.KYCList[0].address}</div>
                      <div>{props.KYCList[0].city}</div>
                      <img src={props.KYCList[0].file} className="py-2" />
                    </div>
                  </div>
                </div>
